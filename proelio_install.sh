...
TMP_FOLDER=$(mktemp -d)
CONFIG_FILE='proelio.conf'
CONFIGFOLDER='/root/.ProelioCore'
COIN_DAEMON='proeliod'
COIN_CLI='proelio-cli'
COIN_PATH='/usr/local/bin/'
COIN_TGZ='https://github.com/zoldur/Proelio/releases/download/v.1.0.0.1/proelio.tar.gz'
...
libminiupnpc-dev libgmp3-dev ufw pkg-config libevent-dev libzmq5 libdb5.3++ unzip >/dev/null 2>&1
if [ "$?" -gt "0" ];
  then
...
bsdmainutils libdb4.8++-dev libminiupnpc-dev libgmp3-dev ufw pkg-config libzqm5 libevent-dev libdb5.3++ unzip"
 exit 1
fi
clear
}
